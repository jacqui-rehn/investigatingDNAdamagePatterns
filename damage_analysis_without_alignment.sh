...
#LOGFILE=$ROOTDIR/weyrich/trimData/damageAnalysisLog.txt
...
#    FASTQcount=$(zcat ${fastq_file} | sed -n '1~2p' | wc -l)
n=1
...
#      COUNT5p=$(zcat ${fastq_file} | sed -n '2~2p' | gawk -F '' '{print $'"$n"'}' | sort -r | uniq -c | awk '{print}' ORS='\t ')
#      echo -e "${fastq_file}\t5p\t${n}\t${FASTQcount}\t${COUNT5p}" >> expanded_base_proportions.txt

COUNT5p=$(zcat ${fastq_file} | sed -n '2~4p' | awk 'length($0) > '"$(($n*2))"'' | gawk -F '' '{print $'"$n"'}' | sort | uniq -c | awk '{print}' ORS='\t')
...
#      COUNT3p=$(zcat ${fastq_file} | sed -n '2~2p' | rev | gawk -F '' '{print $'"$n"'}' | sort -r | uniq -c | awk '{print}' ORS='\t ')
#      echo -e "${fastq_file}\t3p\t${n}\t${FASTQcount}\t${COUNT3p}" >> expanded_base_proportions.txt

COUNT3p=$(zcat ${fastq_file} | sed -n '2~4p' | awk 'length($0) > '"$(($n*2))"'' | rev | gawk -F '' '{print $'"$n"'}' | sort | uniq -c | awk '{print}' ORS='\t')
